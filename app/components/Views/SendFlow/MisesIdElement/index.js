...
class MisesIdElement extends PureComponent {
  static propTypes = {
    /**
     * Name to display
     */
  ...
  render = () => {
    const { onAccountPress, onAccountLongPress, misesId } = this.props;
    const { name, address } = this.state;
    const primaryLabel =
      name && name[0] !== ' ' ? name : shortenAddress(misesId);
...
MisesIdElement.contextType = ThemeContext;

const mapStateToProps = (state) => ({
  network: state.engine.backgroundState.NetworkController.network,
});

export default connect(mapStateToProps)(MisesIdElement);
