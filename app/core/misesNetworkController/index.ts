    ...
    const provider = {
      ...
      sendAsync: (opts: any, cb: (params: any, result: any) => void) => {
        ...
        if (opts.method === 'eth_chainId') {
          cb(null, { result: '0x' + NetworksChainId.mises });
        }
        if (opts.method === 'eth_getBalance') {
          cb(null, { result: 0 });
        }
        if (opts.method === 'eth_getBlockByNumber') {
          cb(null, { result: {} });
        }
        if (
          opts.method === 'eth_call' ||
          opts.method === 'eth_sendTransaction' ||
          opts.method === 'eth_sendTransaction'
        ) {
          cb(
            new Error(
              `Your Metamask wallet is now connected to Mises chain, please switch to ETH chain before invoke ${opts.method}`,
            ),
            null,
          );
        }
      ...
      isConnected: emptyFunction,
      isListening: emptyFunction,
      once: emptyFunction,
